...
def pingpong(n):
    ...
    True
    """
    def inner_helper(k, total=1, direction=1):
        if k == n:
            return total
        elif num_eights(k) > 0 or k % 8 == 0:
            return inner_helper(k + 1, total - direction, -direction)
        else:
            return inner_helper(k + 1, total + direction, direction)
...
def count_coins(change):
    ...
    True
    """
    def inner_helper(change, smallest_coin):
        if change == 0:
            return 1
        if change < 0:
            return 0
        if smallest_coin == None:
            return 0
        with_coin = inner_helper(change - smallest_coin, smallest_coin)
        without_coin = inner_helper(change, next_larger_coin(smallest_coin))
        return with_coin + without_coin
    return inner_helper(change, 1)
